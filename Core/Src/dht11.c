...
void DHT11_Init ( void )
{
	...
	//DHT11_Mode_Out_PP();
    DHT11_DATA_IO_OUT();  //gpio output mode
...
static uint8_t DHT11_ReadByte ( void )
{
	...
	{	 
		...
		//while(DHT11_Data_IN()==Bit_RESET);
          while(DHT11_ReadData()==Bit_RESET);
...
{  
  ...
  uint16_t humi_temp;
	...
	/*输出模式*/
	DHT11_DATA_IO_OUT();//DHT11_Mode_Out_PP();
	/*主机拉低*/
	DHT11_DATA_SetLow();//DHT11_Dout_LOW();
	/*延时18ms*/
	HAL_Delay(20);

	/*总线拉高 主机延时30us*/
	DHT11_DATA_SetHigh();//DHT11_Dout_HIGH(); 

	delay_us(30);   //延时30us

	/*主机设为输入 判断从机响应信号*/ 
	DHT11_DATA_IO_IN();//DHT11_Mode_IPU();
     delay_us(30);   //延时30us
	/*判断从机是否有低电平响应信号 如不响应则跳出，响应则向下运行*/   
	if(DHT11_ReadData()==Bit_RESET)//if(DHT11_Data_IN()==Bit_RESET)     
	{
    /*轮询直到从机发出 的80us 低电平 响应信号结束*/  
    while(DHT11_ReadData() ==Bit_RESET) //while(DHT11_Data_IN()==Bit_RESET);

    /*轮询直到从机发出的 80us 高电平 标置信号结束*/
    while(DHT11_ReadData()==Bit_SET)//while(DHT11_Data_IN()==Bit_SET);
